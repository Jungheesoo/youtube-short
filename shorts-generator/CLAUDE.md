...
- 주인공(중심 인물)은 스토리상 성별이 정해져야 할 특별한 이유가 없으면 항상 젊은 여성으로 설정 (시청자 어필 목적). 원피스 등 세련된 옷차림은 괜찮지만 노출/선정적 묘사는 금지 — 바로 아래 광고주 친화성 규칙과 동일선상.
...
- 렌더링 방식은 항상 "사실적인 사진"으로 고정 — 일러스트/웹툰/전통 채색화/애니메이션풍은 사용하지 않음. 시대/장르에 따라 달라지는 건 배경·의상·소품 묘사뿐이고, 이 톤은 Claude가 `styleGuide` 필드로 매번 새로 결정 (`claude.js` 참고). 실사 질감 자체는 `PHOTOREALISM_STYLE`(같은 파일) 고정 문구가 담당.
