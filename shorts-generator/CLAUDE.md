...
- 렌더링 방식은 항상 `claude.js`의 `STORYBOOK_STYLE` 고정 문구(수채화/색연필 톤 스토리북 일러스트) 하나만 사용 —
  주제(시대/장르)에 따라 화풍 자체를 바꾸지 않음. 사진처럼 보이면 안 됨. 장면마다 달라지는 건 씬별 `timeOfDay`
  (dawn/day/sunset/night/overcast 중 Claude가 선택 → 백엔드가 `TIME_OF_DAY_PALETTE`로 색감 문구 자동 삽입)와
  장소(`LOCATION_VARIETY_RULE`로 다양화 지시)뿐. 인물 구도는 `COMPOSITION_RULE`로 고정(인물이 항상 크게 나오도록).
  imagePrompt 최종 조합 순서: `[장면 묘사] + [COMPOSITION_RULE] + [timeOfDay 색감] + [STORYBOOK_STYLE] + [VERTICAL_SUFFIX]`.
- 배경음악은 채널 고정 테마곡(`backend/assets/music/theme_music.mp3`)을 모든 영상에 동일하게 사용 (`pipeline.js`의 `MUSIC_PATH`, 씬/요청별 오버라이드 없음).
