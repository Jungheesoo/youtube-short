...

{
  ...
  "pinnedComment": "채널 계정으로 달 고정 댓글 텍스트",
  "scenes": [
...
- pinnedComment는 채널 계정이 영상에 직접 남기는 고정 댓글로, 매번 그 영상 내용에 맞게 새로 써(모든 영상에 똑같은 문구를 반복하지 마 — 매일 똑같은 댓글은 시청자에게 성의 없어 보인다). 그 영상의 핵심 통찰/소재를 한 번 더 짚어주면서 시청자의 공감이나 댓글 참여를 자연스럽게 유도하는 1~2문장으로 쓰고, 끝에 어울리는 이모지 1개 정도만 사용(과하게 넣지 마)
...
- 대화는 소재의 핵심 반전일 때만 쓰는 게 아니라, 나레이션 흐름 중 누군가의 말이 조금이라도 등장하면(예: "엄마가 말했다", "친구가 위로하듯 이렇게 말했다", 짧은 인용이라도) 그 대목을 narrator의 간접 서술로 뭉뚱그리지 말고 그 인물의 실제 대사를 담은 캐릭터(characterA/characterB) 씬으로 분리해서 적극적으로 보여줘. 이런 대목이 있으면 대화 씬은 전체 씬 중 1~3개까지 자연스럽게 늘어나도 된다. 다만 소재 자체가 순수 통계/개념/독백형이라 애초에 "누군가의 말"이 등장할 지점이 없는 주제라면 대화를 억지로 끼워 넣지 말고 전부 narrator로 가라 — 지금 추천되는 4개 카테고리(EMOTION_CATEGORIES) 전부에서 매번 대화가 필요한 것은 아니다.
...
조회수를 끌 수 있다면 "부자 되는 사람들의 습관", "성공하는 사람과 그렇지 못한 사람의 차이" 같은
물질적 성공·자기계발형 후킹 소재도 적극 사용해도 좋다.

궁금증 유발(끝까지 보게 만드는 힘): 소재를 고를 때 "결론이 뭘지 궁금해서 끝까지 보고 싶은" 형태인지 항상 점검해.
결론/정답을 topic 문구에 미리 다 드러내지 말고, 아래 방식을 적극 활용해서 정보 격차를 만들어:
  - 비교형: "~하는 사람과 ~하지 않는 사람의 차이" — 둘 중 나는 어느 쪽인지 궁금해지게
  - 개수 명시형: "마음이 따뜻한 사람들의 3가지 습관"처럼 구체적 숫자를 제시해 "그게 뭘까" 궁금하게
  - 사건 암시형: "그날 그 사람이 건넨 말 한마디"처럼 구체적 순간/대사를 암시만 하고 내용은 밝히지 않기
  - 반전 예고형: 처음 예상과는 다른 결론으로 이어질 것 같은 소재(단, 실제 결론은 항상 따뜻하고 담백하게)
단, 이 채널의 잔잔하고 담백한 톤과 광고주 친화성을 해치는 자극적 후킹(선정성, 공포, 충격 반전, 과장된 절규체)은
절대 쓰지 마 — 궁금증은 "잔잔한 여운"의 힘으로 만드는 것이지 자극으로 만드는 게 아니다.
